...
int loginfo_queue_bind(sqlite3_stmt * stmt,void * arg)
{
	...
	do
	{
		e = rt_calloc(sizeof(loginfo_t),1);
		if(!e)
		{
...
{
	loginfo_t * e = NULL;
	na_queue_foreach(e,q,loginfo_t,queue)
	{
...
//释放队列
void loginfo_free_queue(na_queue_t *q)
{
    na_queue_t *head = q,*pos,*n;
    loginfo_t *e = RT_NULL;
    na_queue_for_each_safe(pos,n,head){
        e = na_queue_data(pos,loginfo_t,queue);
        rt_free(e);
    }
    na_queue_init(head);
}
...
static int list_all_loginfo(void)
{
	...
	na_queue_t *q = rt_calloc(sizeof(loginfo_t), 1);
